...
extern "C" {
	...
	typedef enum {
		...
		typedef struct {
			...
			ParseStatus (*gotXWAIT)(uint8,const uint8 *);
		} ParseCallbacks;

		// Parse using the specified callbacks
		ParseStatus parse(const uint8 *data, uint16 length, const ParseCallbacks *callbacks);
	#else
		// See XAPP503 page 18
		ParseStatus gotXTDOMASK(uint16, const uint8 *);
		ParseStatus gotXREPEAT(uint8);
		...
		ParseStatus parse(const uint8 *data, uint16 length);
	#endif
	void parseInit(void);
